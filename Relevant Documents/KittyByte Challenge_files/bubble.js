...
Grader.prototype={
	...
	updateIcon:function(background,icon){
		...
		// Added line
		if(this!=graders[currentProblem]){return}
		this.element.find(".background").attr("fill",colorDef["cyan"]);
		this.element.find(".background").attr("fill",colorDef[background]);
	...
	try{
		switch(serverData.status){
				...
				switch(serverData.status){
					case "correct":
						...
						if(currentProblem > 0) {
							...
							$(".answerlink").show();
							$(".answerlink").html("<a href='http://www.kittybyte.com/coder/kittyanswers/"+problemid+"'>Top Answers (leave this page)</a>/");
						}
						break;
					case "mismatch":
						grader.updateIcon("yellow","unequal")
						grader.updateText("Returned the wrong result. Mouse over me to see test cases.");
						break;
				}
			...
			case "runtime":
				...
				grader.updateText("Submission cannot run. Mouse over me for exception");
				grader.cases=[{
					"color":"orange",
					"name":"Runtime error",
			...
			case "compile":
				...
				grader.updateText("Submission cannot compile. Mouse over me for error");
				grader.cases=[{
					"color":"red",
...
var switchToProblem=function(problemNumber){
	if(currentProblem==null||problemNumber!=currentProblem){
			...
			if(currentProblem>0){
				$.ajax({
					...
					"dataType":"JSON",
			                "success":function(response){
			                        if(response){
							pastTimes[currentProblem] = response;
							if(window.demo){
								var median = -1;
								times = pastTimes[currentProblem];
								if(times.length>0) {
									times.sort(function(a,b) {return a - b;} );
									var half = Math.floor(times.length/2);
									if(times.length % 2) median = times[half];
									else median = (times[half-1] + times[half]) / 2.0;
								}
								avgTime=currentProblem==0?0:median;
								problems[currentProblem].avgTime=avgTime;
								var minutes=Math.floor(avgTime/1000)%60;
								done=0;
								timerElement.find(".avgTime").text(Math.floor(avgTime/60000)+":"+(minutes<10?"0":"")+minutes+"."+Math.floor(avgTime%1000/100)+" Median");
								if(avgTime < 0 || avgTime == 1e10) {
									avgTime = 1e10;
									timerElement.find(".avgTime").text("No top score yet!");
								}
							}
			                        }
			                        else{
		...
		}else if(problems[currentProblem].avgTime != 0){
			avgTime = problems[currentProblem].avgTime
			var minutes=Math.floor(avgTime/1000)%60;
			timerElement.find(".avgTime").text(Math.floor(avgTime/60000)+":"+(minutes<10?"0":"")+minutes+"."+Math.floor(avgTime%1000/100)+" Median");
			if(avgTime < 0 || avgTime == 1e10) {
				avgTime = 1e10;
		...
		if(graders[currentProblem].successful && currentProblem > 0) {
			...
			$(".answerlink").show();
			$(".answerlink").html("Top Answers link enabled when you get this right. Register for an account to receive email updates <a href='https://www.kittybyte.com/register/'>link</a>");
		}else {
			$(".answerlink").hide();
		}
	}
